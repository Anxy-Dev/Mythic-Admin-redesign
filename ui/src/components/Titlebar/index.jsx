...
export default () => {
	...
	useEffect(() => {
		// Listen for quick features updates
		const handleStorageChange = () => {
			setQuickFeatures(JSON.parse(localStorage.getItem('adminQuickFeatures') || '[]'));
		};
	...
	const quickFeatureActions = {
		'invisibility': { icon: ['fas', 'eye-slash'], action: goInvisible, minLevel: 100 },
		'player-ids': { icon: ['fas', 'id-badge'], action: toggleIds, minLevel: 0 },
		'noclip': { icon: ['fas', 'ghost'], action: () => Nui.send('ToggleNoClip', { active: true }), minLevel: 75 },
	};
		...
		>
			<Toolbar disableGutters>
				...
				<div className={classes.right}>
					...
					<Divider orientation="vertical" flexItem />
					{quickFeatures.map((featureId) => {
						const feature = quickFeatureActions[featureId];
						if (feature && permissionLevel >= feature.minLevel) {
							return (
								<Tooltip key={featureId} title={featureId.replace('-', ' ')}>
									<IconButton onClick={feature.action}>
					...
					{quickFeatures.length > 0 && <Divider orientation="vertical" flexItem />}
